...
_in_flight_requests_count = metrics.register_gauge(
    "_in_flight_requests_count", labels=["method", "servlet"]
)
...
def _collect_in_flight():
    ...
    for (method, name), count in counts.iteritems():
        _in_flight_requests.set(0, method, name)
...
class _RequestStats(object):
    ...
    def from_context(context):
        ...
        self.ru_utime, self.ru_stime = context.get_resource_usage()
        self.db_txn_count = context.db_txn_count
        self.db_txn_duration_ms = context.db_txn_duration_ms
        self.db_sched_duration_ms = context.db_sched_duration_ms
